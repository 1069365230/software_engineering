...
@NoArgsConstructor
@Entity
@Table(name = "ATTENDING", uniqueConstraints={
        @UniqueConstraint(columnNames = {"global_attendee_id", "global_event_id"})
})
public class AttendingEvent {
    @Id
    @Column(name = "attending_event_id")
    @SequenceGenerator(
            name = "attending_event_id_sequence",
            sequenceName = "attending_sequence"
    )
    @GeneratedValue(
            strategy = GenerationType.SEQUENCE,
            generator = "attending_sequence"
    )
    private Integer id;
